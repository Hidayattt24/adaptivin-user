...
export function middleware(request: NextRequest) {
  ...
  const hasSeenSplash = request.cookies.get(`${COOKIE_PREFIX}hasSeenSplash`)?.value;
...
export const config = {
  matcher: [
     ...
     * - public folder files
     */
    "/((?!api|_next/static|_next/image|favicon.ico|logo|guru|siswa|pick-role|pixel|.*\\.svg|.*\\.png|.*\\.jpg|.*\\.jpeg|.*\\.webp|.*\\.gif).*)",
  ],
};
