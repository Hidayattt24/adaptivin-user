...
const BankSoalPage = () => {
  ...
  // Convert Question array from backend data
  const questions: Question[] = useMemo(() => {
    return soalList.map(convertSoalToQuestion);
  }, [soalList]);
  ...
  const handleSaveEdit = async (updatedQuestion: Question) => {
    try {
      ...
      const payload = {
        ...
        durasi_soal: updatedQuestion.timeValue,
        jawaban: jawaban,
      };

      await updateSoalMutation.mutateAsync({
        soal_id: updatedQuestion.id,
        payload: payload,
      });
  ...
  const handleDelete = async (question: Question, number: number) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteSoalMutation.mutateAsync(question.id);
